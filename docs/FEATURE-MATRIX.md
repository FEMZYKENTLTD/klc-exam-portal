...
**Date:** 2026-09-05 (matrix) · **Updated:** 2026-09-06 · **Branch:**
...
CI results. **Directive builds landed after the audit:** subject catalogue,
H2 AES, mock mode, searchable subjects, exam calendar, arm balancing,
practice/mock review, backup/restore/USB pack, invigilator web page, global
dark mode, web question upload — this matrix tracks the CURRENT code.
...
| Super Admin pre-created at install | ✅ | H2 offline seeded from config `app.superadmin.email/password` (BCrypt) with forced-change guidance; cloud bootstrap `ensurePostgresSuperAdmin` seeds SUPER_ADMIN from the same config (CI supplies via `KLC_SUPERADMIN_EMAIL/PASSWORD` secrets) — **test-verified** (`AuthServiceTest.seededOfflineSuperAdminCanLogin`); no committed/printed production password |
...
| Pre-loaded subjects catalogue (full JSS1–3 + SS1–3 coverage) | ✅ | **135-row catalogue** (19 per JSS level ×3 + 26 per SSS level ×3, incl. Geography, Further Maths, Civic Ed, all core JSS subjects for JSS1–JSS3) auto-ensured on every H2 startup + idempotent cloud migration — **test-verified** (`SubjectCatalogueTest`: coverage/idempotency/codes) |
...
| Web admin teacher question upload | ✅ | `klc-web-admin/question_upload.html`+`.js`: staff login, plain-text/CSV-style parser with live preview, subject/class selection (code+subject list), upload via `staff_upload_questions` RPC (bcrypt + teacher subject-ownership + is_approved=FALSE for the existing approval workflow) |
...
| Class-arm auto-balancing tool | ✅ | `util.ArmBalancer` deterministic planner + `ClassManagerController` Preview/Apply (transactional, audited per move) — **test-verified** (`ArmBalancerTest` 6 cases) |
...
| Exam calendar view | ✅ | `ExamManager` "📅 Exam Calendar" dialog: 42-day holiday-aware strip (school days, exam load, overload flags, closed days) + ordered schedule — **test-verified** (`ExamCalendarTest`/`SchoolHolidays` Nigerian statutory + config extras) |
...
| Practice/past-questions library with solution review | ✅ | Practice + MOCK answer review dialog after submission (question, your answer, correct answer, topic, RIGHT/WRONG); correct answers NEVER shown for official exams |
| Mock exam mode (no ranking) | ✅ | Distinct `exams.is_mock` mode: timed/exam-style, proctoring off, `MOCK_SUBMITTED` attempts, NO official `results` row, [MOCK EXAM - NOT RECORDED] labels, excluded via `v_official_exams`; dashboard offers a scheduled mock only when no official exam is live (with confirm) |
...
| Invigilator mobile monitor web page | ✅ | `klc-web-admin/invigilator.html`+`.js` responsive live room over `staff_live_room` RPC (bcrypt per call): active official attempts, elapsed, strikes; 15 s auto-refresh; practice/mock excluded |
...
| 1-click encrypted `.klcbackup` | ✅ | `BackupService.createBackup` full snapshot (every app table, streaming) AES-256-GCM when `backup.key` set, SHA-256 logged — **test-verified** round-trip incl. encrypted restore path (`BackupServiceTest`) |
| Auto-backup scheduler | 🟡 | In-app 24-h `Timeline` auto-backup (`BackupController`); OS-level Task Scheduler documented in `INSTALLER_GUIDE.md` (external scheduling, not embedded) |
| Backup restore / USB recovery pack / PITR | ✅ | `BackupService.restore()` replays every table in ONE transaction (FK suspension per engine, full rollback); `exportUsbPack()` writes snapshot + SHA-256 + printed manual; `BackupController` Restore-from-Backup & Export-USB buttons — **test-verified** (`BackupServiceTest`: round-trip w/ commas/quotes/newlines/NULLs, corrupt-file rollback, USB pack). Native Supabase PITR documented as Pro-tier path (dashboard), app-level pack is the strongest free-tier recovery — honestly labelled |
...
BCrypt ✅ (tests) · PreparedStatements ✅ (pervasive; param SQL) · RLS/JWT 🟠 (cloud RLS + anon JWT for web; desktop role bypasses by design) · RBAC 🟡 · **Encrypted H2 cache AES ✅** (`h2.encryption=aes` + `CIPHER=AES`, 16+ char file key; plaintext→AES one-time migration; fail-closed on missing/short key — **test-verified** `H2EncryptionTest`: round-trip/restart/modify, wrong-key fails, migration preserves data, no plaintext fallback) · TLS ✅ (Supabase endpoints) · WORM audit ✅ (cloud trigger) · IP whitelist 🟡 · session timeout ✅ · 2FA TOTP 🟡 · NDPR/GDPR 🟠 (no explicit data-retention/deletion workflows) · QR anti-forgery 🟠 · malpractice evidence storage ✅ (logs + webcam evidence dir) · **Compromised-code remediation ✅** (fail-closed registration, PR #3).

## 11. UI/UX

Navy/gold theme + premium CSS ✅ · WCAG 2.1 AA ⚠️ (no formal compliance test) · dark/light toggle ✅ (global `ThemeService` + `klc-dark.css` on every scene; persisted preference; toggles on login + student + admin dashboards) · 1024×768 responsive/touchscreen 🟡 · font 4 levels/high-contrast/dyslexic/zoom ✅ (exam) · keyboard-navigable CBT ✅ · branding rules ✅ (see §2) · animations 🟡.

## 12. TECH STACK

Java 17 + JavaFX 17 ✅ · Hibernate 6 (dependency only — app uses JDBC) 🟠 · H2 ✅ · iText 8 PDF ✅ · PDFBox 3 ✅ · POI ✅ · BCrypt ✅ · Maven ✅ · jlink/jpackage 🟠 (CI Windows app-image; see F3) · Supabase PG15/Storage/PostgREST ✅ schema+migrations · Realtime ⚠️ (WS endpoints config-driven, best-effort) · Web admin vanilla JS ✅ · **"Database Schema – 18 tables" list is outdated** — repo ships a **33-table authoritative schema** (see `docs/SCHEMA-INVENTORY.md`); the spec's 18-table list is stale and is not used as the target · API docs ✅ `API.md`.
...
**F2 — RESOLVED: Pre-loaded subject seed.** The full catalogue is now
auto-ensured: 135 rows (19 subjects per JSS level, 26 per SSS level incl.
Geography, Further Maths, Civic Education and every core JSS subject for
JSS1–JSS3) inserted idempotently on every H2 startup AND in the always-run
cloud migration (`subject_code` UNIQUE guard) — see `SubjectCatalogueTest`.

**F3 — PARTIALLY RESOLVED / external build dependency documented.** x64
zero-JDK Windows packaging is the production target (CI `v*`-tag job
produces the jpackage app-image). x86 and Win7/8/8.1 require a separate
Java-8 runtime track and an x86/legacy build environment; that environment
is an external dependency and is documented precisely in
`INSTALLER_GUIDE.md` — the targets are preserved, not deleted.

**F4 — Stands as documented (platform reality, not downgraded).** Pure
JavaFX cannot block Alt-Tab/Win/Task Manager at the OS level; the app
implements the strongest viable subset (full-screen, focus-loss 3-strike
detection, clipboard/context-menu blocking, malpractice logging).

**F5 — RESOLVED: Encrypted H2 cache (AES).** Real `CIPHER=AES` at rest via
`h2.encryption=aes` + `h2.cipher.filePassword` (16+ chars), fail-closed
when unset, one-time plaintext→AES migration that preserves data —
verified by `H2EncryptionTest` (round-trip/restart/modify, wrong key fails
safely, no plaintext fallback cache).

**F6 — RESOLVED (implementation, not docs):** visual exam calendar
(`ExamManager` 📅 dialog + holiday-aware planner); class-arm auto-balancing
(`ArmBalancer` + `ClassManager` Preview/Apply with audit); practice/mock
solution review; distinct `is_mock` exam mode never recorded officially;
invigilator live-room web page (`klc-web-admin/invigilator.html`);
backup restore + USB recovery pack + full-snapshot backups with real
transactional restore; global dark/light theme (persisted); web-admin
question upload (`question_upload.html` + RPC). OS Task-Scheduler
integration remains external to the app (documented); Supabase-native PITR
is the Pro-tier dashboard path, honestly labelled.

**F7 — RESOLVED: authoritative inventory.** The real schema is 33
application tables; see `docs/SCHEMA-INVENTORY.md` (per-table purpose, PK,
FKs, constraints, business rules). The old "18 tables" list is not used as
the target.
