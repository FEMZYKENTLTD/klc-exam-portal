...
/**
 * ResultViewerController - KLC CBT Suite v1.0
 *
 * RULE 11 FIXES vs previous version:
 * 1. Rule 5 VIOLATION FIXED: was using raw
 *    ps.setObject(UUID.fromString(...)) which CRASHES on H2 offline mode
 *    (H2 stores UUIDs as VARCHAR(36)). Now AuthService.setUuid everywhere.
 * 2. Session no longer hardcoded to "2024/2025" - read from
 *    school_profile.session_current with a safe fallback.
 * ALL features preserved: results table, term filter, report card,
 * transcript, graduation certificate, appeals.
 */
public class ResultViewerController {
    ...
    public static class R {
        ...
        R(String rid, String s, String sc, String g, String d) {
            resultId = rid; subject = s; score = sc; grade = g; date = d;
        }
    ...
    public void initialize() {
        ...
        colScore.setCellValueFactory(c ->
            new javafx.beans.property.SimpleStringProperty(c.getValue().getScore()));
        colGrade.setCellValueFactory(c ->
            new javafx.beans.property.SimpleStringProperty(c.getValue().getGrade()));
        ...
        table.getSelectionModel().selectedItemProperty().addListener((o, a, b) -> {
            if (b != null && appealSubjectField != null)
                appealSubjectField.setText(b.subject);
        });
    ...
    void load() {
        ...
        try (Connection c = DatabaseManager.getConnection();
            ...
            // FIX Rule 5: setUuid instead of raw setObject(UUID)
            ...
            summaryLabel.setText(cnt == 0
                ? "No results yet - take an exam!"
                : String.format(
                    "Lifetime Results: %d exams | Cumulative Average: %.1f%% " +
                    "| Result PIN: SURNAME+CLASS", cnt, sum / cnt));
        } catch (Exception e) {
    ...
    private void downloadReport() {
        try {
            String term = termBox != null && termBox.getValue() != null
                ? termBox.getValue() : "1st";
            String out = ReportCardPdf.generateForStudent(
                AuthService.Session.userId, null, term, currentSession());
            new Alert(Alert.AlertType.INFORMATION,
                "Official KLC Term Report Card saved:\n" + out +
    ...
    private void downloadTranscript() {
        try {
            ...
            new Alert(Alert.AlertType.INFORMATION,
                "Cumulative Transcript JSS1-SS3 saved:\n" + out +
    ...
    @FXML
    private void downloadGraduation() {
        try {
            String out = GraduationCertificatePdf.generate(
                AuthService.Session.userId, null);
            new Alert(Alert.AlertType.INFORMATION,
                "Graduation Certificate saved:\n" + out +
    ...
    private void submitAppeal() {
        ...
        if (sel == null || appealText == null
                || appealText.getText().trim().isEmpty()) {
            new Alert(Alert.AlertType.WARNING,
                "Select a result row and enter your appeal reason.").show();
            return;
        }
        try (Connection c = DatabaseManager.getConnection();
             PreparedStatement ps = c.prepareStatement(
            ...
            ps.executeUpdate();
            new Alert(Alert.AlertType.INFORMATION,
                "Appeal submitted. Admin will review. You will be notified.").show();
            appealText.clear();
